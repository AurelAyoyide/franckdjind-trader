      ...
      <div className="grid gap-4">
        {pagedPosts.items.map((post) => (
          <article className="rounded-lg border border-line bg-surface p-5" key={post.id}>
            ...
            <details className="mt-5 rounded-lg border border-line bg-foreground/[0.04] p-4"><summary className="cursor-pointer text-sm font-black">Modifier la publication</summary><CommunityPostEditForm post={{ id: post.id, title: post.title, body: sanitizeCommunityHtml(post.body), courseId: post.courseId, pinned: post.pinned, commentsEnabled: post.commentsEnabled }} courses={courses.map((course) => ({ id: course.id, title: course.title }))} /></details>
