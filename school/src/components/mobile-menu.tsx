...
export function MobileMenu({
  ...
  return (
    <div className="flex items-center gap-2 lg:hidden">
      <ThemeToggle label={t("Changer le theme")} />
      <LanguageSwitch currentPathname={currentPathname} currentSearch={currentSearch} locale={locale} />
      <details className="group">
        <summary
          aria-label={t("Ouvrir le menu")}
          className="mobile-menu-summary inline-flex h-10 w-10 cursor-pointer items-center justify-center rounded-lg border border-line bg-foreground/[0.06] text-foreground"
        >
          <Menu className="mobile-menu-icon-open h-5 w-5" aria-hidden="true" />
          <X className="mobile-menu-icon-close hidden h-5 w-5" aria-hidden="true" />
        </summary>
